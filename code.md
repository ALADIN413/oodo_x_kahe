...
```text
PORT=3000
MONGO_URL=mongodb+srv://arishexim011_db_user:ircPUoCrTYsNq0jI@cluster0.k4jqzyu.mongodb.net/?appName=Cluster0
JWT_SECRET=traveloop-dev-secret-key-change-in-production
GEMINI_API_KEY=AIzaSyBOsER6NC-MtUxHS3BjXR6kagGXVcjIN9w
...
export async function createTrip(req: AuthRequest, res: Response) {
  try {
    const {
      groupId, destination, startDate, endDate, budget, interests,
      headcount, tripType, constraints,
    } = req.body;
...
export async function generateQuestionsForTrip(req: AuthRequest, res: Response) {
  try {
    const { tripId } = req.body;
    const trip = await Trip.findById(tripId);
    if (!trip) return res.status(404).json({ error: 'Trip not found' });

    const questions = await generateQuestions(trip);
    trip.set('questions', questions.map((q: any) => ({ question: q.question, answer: '' })));
    await trip.save();

    res.json({ questions: trip.questions });
  } catch (error) {
...
export async function submitAnswers(req: AuthRequest, res: Response) {
  try {
    ...
    const aiPlanJson = await generateFinalItinerary(trip);
...
export async function regenerateDayForTrip(req: AuthRequest, res: Response) {
  try {
    ...
    const newDay = await regenerateDay(trip, dayNumber, instruction);
...
import { Router } from 'express';
import {
  createTripSchema, generateQuestionsSchema, submitAnswersSchema, regenerateDaySchema, validate,
} from '../middleware/validation.js';
import { authMiddleware } from '../middleware/auth.js';
import {
  createTrip, generateQuestionsForTrip, submitAnswers, regenerateDayForTrip, getTripsByGroup,
} from '../controllers/trip.controller.js';

const router = Router();

router.post('/create', authMiddleware, validate(createTripSchema), createTrip);
router.post('/generate-questions', authMiddleware, validate(generateQuestionsSchema), generateQuestionsForTrip);
...
export async function generateQuestions(trip: any): Promise<Array<{ question: string }>> {
  const days = Math.ceil(
    (new Date(trip.endDate).getTime() - new Date(trip.startDate).getTime()) / (1000 * 60 * 60 * 24)
  ) + 1;

  const systemPrompt = `You are a smart travel planning assistant. Your job is to ask follow-up questions to personalize a trip itinerary. 
You must respond with ONLY a JSON array of 3-5 questions. No other text. No markdown.`;

  const userPrompt = `Trip details so far:
- Destination: ${trip.destination}
- Dates: ${new Date(trip.startDate).toLocaleDateString()} to ${new Date(trip.endDate).toLocaleDateString()} (${days} days)
- Budget: $${trip.budget} total
- People: ${trip.headcount}
- Interests: ${(trip.interests || []).join(', ') || 'general'}
- Trip type: ${trip.tripType || 'general'}
- Constraints: ${Object.entries(trip.constraints || {}).filter(([_, v]) => v).map(([k]) => k).join(', ') || 'none'}

Generate 3-5 smart follow-up questions to help create the perfect itinerary. 
Examples: hotel preferences? restaurant types? activity pace? transport? nightlife? family needs?
...
["question 1?", "question 2?", "question 3?"]`;
  ...
  try {
    ...
    if (!text) return getFallbackQuestions();
...
function getFallbackQuestions() {
  return [
    { question: 'What type of accommodation do you prefer? (hotel, hostel, resort, Airbnb)' },
    { question: 'Do you want sightseeing-heavy days or a more relaxed pace?' },
    { question: 'Any specific cuisines you want to try?' },
    { question: 'Need local transport arranged or prefer walking/public transit?' },
    { question: 'Should we include evening/nightlife activities?' },
  ];
}

export async function generateFinalItinerary(trip: any): Promise<any> {
  const days = Math.ceil(
    (new Date(trip.endDate).getTime() - new Date(trip.startDate).getTime()) / (1000 * 60 * 60 * 24)
  ) + 1;

  const qaContext = (trip.questions || [])
  ...
  const userPrompt = `Create a ${days}-day group itinerary with the following context:
...
Trip type: ${trip.tripType || 'general'}
Dietary: ${trip.constraints?.vegetarian ? 'Vegetarian options needed' : 'No dietary restrictions'}
Family: ${trip.constraints?.kids ? 'Include kid-friendly activities' : ''} ${trip.constraints?.seniors ? 'Accessible for seniors' : ''}
Style: ${trip.constraints?.luxury ? 'Luxury experience' : 'Budget-friendly'}
Transport: ${trip.constraints?.publicTransport ? 'Prefer public transport' : 'Flexible transport'}

Additional context from user preferences:
${qaContext || 'No additional preferences specified.'}
...
Respond with valid JSON following this EXACT structure (no markdown, no code fences):
{
  ...
  try {
    const text = await callGemini(systemPrompt, userPrompt);
    if (!text) return getFallbackEnhancedItinerary();
    return JSON.parse(text);
  } catch {
    return getFallbackEnhancedItinerary();
  }
}

export async function regenerateDay(trip: any, dayNumber: number, instruction: string): Promise<any> {
  const systemPrompt = `You are a travel planner. Regenerate a single day of an itinerary. Respond with JSON only.`;

  const currentDays = JSON.stringify(trip.aiPlanJson?.days || []);
...
function getFallbackEnhancedItinerary() {
  return {
    days: [
      {
        day: 1,
        city: 'Destination',
        activities: [
          { time: '10:00', activity: 'Arrival & Check-in', description: 'Check into accommodation and freshen up', cost: 0, category: 'other' },
          { time: '12:30', activity: 'Welcome Lunch', description: 'Group lunch at a local restaurant', cost: 25, category: 'food' },
          { time: '14:00', activity: 'City Tour', description: 'Explore main attractions and landmarks', cost: 20, category: 'attraction' },
          { time: '19:00', activity: 'Group Dinner', description: 'Dinner at recommended restaurant', cost: 35, category: 'food' },
        ],
      },
      {
        day: 2,
        city: 'Destination',
        activities: [
          { time: '08:00', activity: 'Breakfast', description: 'Hotel breakfast', cost: 12, category: 'food' },
          { time: '09:00', activity: 'Adventure Activity', description: 'Outdoor adventure experience', cost: 75, category: 'attraction' },
          { time: '12:30', activity: 'Lunch', description: 'Lunch near activity spot', cost: 18, category: 'food' },
          { time: '14:00', activity: 'Cultural Visit', description: 'Museum or historical site', cost: 15, category: 'attraction' },
          { time: '17:00', activity: 'Shopping', description: 'Local markets', cost: 40, category: 'shopping' },
          { time: '20:00', activity: 'Evening Activity', description: 'Nightlife or entertainment', cost: 35, category: 'other' },
        ],
      },
      {
        day: 3,
        city: 'Destination',
        activities: [
          { time: '09:00', activity: 'Brunch', description: 'Leisurely brunch', cost: 20, category: 'food' },
          { time: '11:00', activity: 'Final Sightseeing', description: 'Must-see attractions', cost: 15, category: 'attraction' },
          { time: '14:00', activity: 'Departure', description: 'Check out and transfer', cost: 30, category: 'transport' },
        ],
      },
    ],
    totalCost: 340,
    currency: 'USD',
    budgetBreakdown: { accommodation: 150, food: 110, transport: 30, activities: 50, other: 0 },
    recommendations: {
      hotel: 'Mid-range city hotel near main attractions',
      restaurants: ['Local cuisine restaurant', 'International food court'],
      transportTips: 'Taxis and ride-sharing are readily available',
    },
    notes: 'Sample itinerary. Costs are estimates per person.',
  };
}
...
```text
EXPO_PUBLIC_API_URL=http://192.168.65.149:3000
...
export default function TripDetailScreen() {
  ...
  async function handleExportPDF() {
    ...
    const html = `
      <html>
        ...
        <body>
          ...
          <p class="sub">${new Date(trip.startDate).toLocaleDateString()} - ${new Date(trip.endDate).toLocaleDateString()}</p>
          <div class="meta">
    ...
    try {
      ...
      const pdfName = `${trip.destination.replace(/\s+/g, '_')}_Itinerary.pdf`;
...
const INTEREST_OPTIONS = ['Adventure', 'Food', 'Nature', 'Nightlife', 'History', 'Shopping', 'Relaxation', 'Photography'];
const TRIP_TYPES = [
  ...
  async function handleCreateTrip() {
    ...
    try {
      const trip = await createTrip({
        ...
        tripType,
        constraints: Object.fromEntries(Object.entries(constraints).filter(([_, v]) => v)),
      });
      router.replace(`/group/${id}/trip/${trip._id}/follow-up`);
    } catch (e: any) {
  ...
  return (
    <ScrollView style={[styles.container, { backgroundColor: colors.background }]}>
      <View style={styles.content}>
        ...
        <Text style={[styles.subtitle, { color: colors.textSecondary }]}>
          Our AI will ask a few questions to personalize your itinerary
        </Text>
        ...
        <View style={[styles.card, { backgroundColor: colors.surface, borderColor: colors.border }]}>
          ...
          <Text style={[styles.label, { color: colors.textSecondary, marginTop: 8 }]}>Trip Type</Text>
            ...
            {TRIP_TYPES.map((t) => (
              <TouchableOpacity
                ...
                style={[
                  styles.optionChip,
                  {
                    backgroundColor: tripType === t.value ? colors.primary : colors.surfaceAlt,
                    borderColor: tripType === t.value ? colors.primary : colors.border,
                  },
                ]}
                onPress={() => setTripType(t.value)}
              >
                <Text style={[styles.optionChipText, { color: tripType === t.value ? '#FFF' : colors.textSecondary }]}>
                  {t.label}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <Text style={[styles.sectionTitle, { color: colors.text, marginTop: 20 }]}>Interests</Text>
            ...
            {INTEREST_OPTIONS.map((item) => (
              <TouchableOpacity
                ...
                style={[
                  styles.chip,
                  {
                    backgroundColor: interests.includes(item) ? colors.primary : colors.surfaceAlt,
                    borderColor: interests.includes(item) ? colors.primary : colors.border,
                  },
                ]}
                onPress={() => toggleInterest(item)}
              >
                <Text style={[styles.chipText, { color: interests.includes(item) ? '#FFF' : colors.textSecondary }]}>
                  {item}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          <Text style={[styles.sectionTitle, { color: colors.text, marginTop: 20 }]}>Constraints</Text>
          <Text style={[styles.hint, { color: colors.textSecondary }]}>
            Helps the AI plan better
          </Text>
          <View style={styles.constraintGrid}>
            {[
              { key: 'vegetarian' as const, label: '🥗 Vegetarian' },
              { key: 'kids' as const, label: '👶 With Kids' },
              { key: 'seniors' as const, label: '👴 Senior Friendly' },
              { key: 'luxury' as const, label: '✨ Luxury' },
              { key: 'publicTransport' as const, label: '🚌 Public Transport' },
            ].map((c) => (
              <TouchableOpacity
                key={c.key}
                style={[
                  ...
                  {
                    backgroundColor: constraints[c.key] ? colors.primary : colors.surfaceAlt,
                    borderColor: constraints[c.key] ? colors.primary : colors.border,
                  },
                ]}
                onPress={() => toggleConstraint(c.key)}
              >
                <Text style={[styles.constraintText, { color: constraints[c.key] ? '#FFF' : colors.text }]}>
                  {c.label}
                </Text>
              </TouchableOpacity>
            ))}
          ...
          >
            <Text style={styles.buttonText}>
              {loading ? 'Creating...' : 'Continue → AI Questions'}
            </Text>
          </TouchableOpacity>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  hint: { fontSize: 12, marginTop: -6, marginBottom: 10 },
  label: { fontSize: 13, fontWeight: '600', marginBottom: 6, marginTop: 12 },
  input: { height: 48, borderRadius: 12, borderWidth: 1, paddingHorizontal: 16, fontSize: 16 },
  ...
  constraintGrid: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  constraintChip: { paddingHorizontal: 14, paddingVertical: 10, borderRadius: 12, borderWidth: 1 },
  constraintText: { fontSize: 13, fontWeight: '600' },
...
# FILE: frontend/assets/adaptive-icon.png


# FILE: frontend/assets/favicon.png


# FILE: frontend/assets/icon.png


# FILE: frontend/assets/splash.png
...
export const api = {
  ...
  generateQuestions: (token: string, tripId: string) =>
    authRequest('/trips/generate-questions', token, { method: 'POST', body: JSON.stringify({ tripId }) }),
...
interface AppDataContextType {
  ...
  generateQuestions: (tripId: string) => Promise<any>;
...
export function AppDataProvider({ children }: { children: React.ReactNode }) {
  ...
  const generateQuestions = useCallback(async (tripId: string) => {
    const data = await api.generateQuestions(token!, tripId);
    return data;
  }, [token]);
  ...
  return (
    <AppDataContext.Provider value={{
      ...
      generateQuestions, submitAnswers, regenerateDay, getTrips,
    }}>
      {children}
...
importers:
  ...
  frontend:
    dependencies:
      '@expo/vector-icons':
        specifier: ^14.0.0
        version: 14.1.0(expo-font@14.0.11(expo@54.0.33)(react-native@0.81.5(@babel/core@7.29.0)(@types/react@19.1.17)(react@19.1.0))(react@19.1.0))(react-native@0.81.5(@babel/core@7.29.0)(@types/react@19.1.17)(react@19.1.0))(react@19.1.0)
      ...
      expo-file-system:
        specifier: ^55.0.19
        version: 55.0.19(expo@54.0.33)(react-native@0.81.5(@babel/core@7.29.0)(@types/react@19.1.17)(react@19.1.0))
      expo-font:
        specifier: ~14.0.10
      ...
      expo-print:
        specifier: ^55.0.14
        version: 55.0.14(expo@54.0.33)(react-native@0.81.5(@babel/core@7.29.0)(@types/react@19.1.17)(react@19.1.0))
      expo-router:
        specifier: ~6.0.17
      ...
      expo-sharing:
        specifier: ^55.0.18
        version: 55.0.18(expo@54.0.33)(react-native@0.81.5(@babel/core@7.29.0)(@types/react@19.1.17)(react@19.1.0))(react@19.1.0)
      expo-splash-screen:
        specifier: ~31.0.12
    ...
    devDependencies:
      ...
      babel-preset-expo:
        specifier: ~54.0.10
        version: 54.0.10(@babel/core@7.29.0)(@babel/runtime@7.28.6)(expo@54.0.33)(react-refresh@0.18.0)
      typescript:
        specifier: ^5.7.0
        version: 5.9.3
...
packages:
  ...
  expo-print@55.0.14:
    resolution: {integrity: sha512-GNznEjRHUjJj3GRRElHff3t2xtarO1zakEBdlxuYBoY0G/Ae59XaBQdlpzTjhceAdASlqsrnugHlQz6bsZRDnw==}
    peerDependencies:
      expo: '*'
      react-native: '*'
  ...
  expo-sharing@55.0.18:
    resolution: {integrity: sha512-Tqy4LXRLw/UEg5mT7BKhx8y4ReNz8fVldvhHJV5cesH3kRgEerHkYxVwid2vd7v34KnNp0RH1OqUyDlzZTQ9AQ==}
    peerDependencies:
      expo: '*'
      react: '*'
      react-native: '*'
...
snapshots:
  ...
  '@expo/cli@54.0.23(expo-router@6.0.23)(expo@54.0.33)(react-native@0.81.5(@babel/core@7.29.0)(@types/react@19.1.17)(react@19.1.0))(typescript@5.9.3)':
    dependencies:
      ...
      ws: 8.20.0
    optionalDependencies:
      expo-router: 6.0.23(@expo/metro-runtime@6.1.2)(@types/react-dom@19.2.3(@types/react@19.1.17))(@types/react@19.1.17)(expo-constants@18.0.13)(expo-linking@8.0.11)(expo@54.0.33)(react-dom@19.1.0(react@19.1.0))(react-native-gesture-handler@2.28.0(react-native@0.81.5(@babel/core@7.29.0)(@types/react@19.1.17)(react@19.1.0))(react@19.1.0))(react-native-reanimated@4.1.7(react-native-worklets@0.5.1(@babel/core@7.29.0)(react-native@0.81.5(@babel/core@7.29.0)(@types/react@19.1.17)(react@19.1.0))(react@19.1.0))(react-native@0.81.5(@babel/core@7.29.0)(@types/react@19.1.17)(react@19.1.0))(react@19.1.0))(react-native-safe-area-context@5.6.2(react-native@0.81.5(@babel/core@7.29.0)(@types/react@19.1.17)(react@19.1.0))(react@19.1.0))(react-native-screens@4.16.0(react-native@0.81.5(@babel/core@7.29.0)(@types/react@19.1.17)(react@19.1.0))(react@19.1.0))(react-native-web@0.21.2(react-dom@19.1.0(react@19.1.0))(react@19.1.0))(react-native@0.81.5(@babel/core@7.29.0)(@types/react@19.1.17)(react@19.1.0))(react@19.1.0)
  ...
  '@expo/vector-icons@14.1.0(expo-font@14.0.11(expo@54.0.33)(react-native@0.81.5(@babel/core@7.29.0)(@types/react@19.1.17)(react@19.1.0))(react@19.1.0))(react-native@0.81.5(@babel/core@7.29.0)(@types/react@19.1.17)(react@19.1.0))(react@19.1.0)':
    dependencies:
  ...
  babel-preset-expo@54.0.10(@babel/core@7.29.0)(@babel/runtime@7.28.6)(expo@54.0.33)(react-refresh@0.18.0):
    dependencies:
      '@babel/helper-module-imports': 7.28.6
      '@babel/plugin-proposal-decorators': 7.29.0(@babel/core@7.29.0)
      '@babel/plugin-proposal-export-default-from': 7.27.1(@babel/core@7.29.0)
  ...
  expo-file-system@55.0.19(expo@54.0.33)(react-native@0.81.5(@babel/core@7.29.0)(@types/react@19.1.17)(react@19.1.0)):
    dependencies:
      expo: 54.0.33(@babel/core@7.29.0)(@expo/metro-runtime@6.1.2)(expo-router@6.0.23)(react-native@0.81.5(@babel/core@7.29.0)(@types/react@19.1.17)(react@19.1.0))(react@19.1.0)(typescript@5.9.3)
      react-native: 0.81.5(@babel/core@7.29.0)(@types/react@19.1.17)(react@19.1.0)
  ...
  expo-print@55.0.14(expo@54.0.33)(react-native@0.81.5(@babel/core@7.29.0)(@types/react@19.1.17)(react@19.1.0)):
    dependencies:
      expo: 54.0.33(@babel/core@7.29.0)(@expo/metro-runtime@6.1.2)(expo-router@6.0.23)(react-native@0.81.5(@babel/core@7.29.0)(@types/react@19.1.17)(react@19.1.0))(react@19.1.0)(typescript@5.9.3)
      react-native: 0.81.5(@babel/core@7.29.0)(@types/react@19.1.17)(react@19.1.0)
  ...
  expo-sharing@55.0.18(expo@54.0.33)(react-native@0.81.5(@babel/core@7.29.0)(@types/react@19.1.17)(react@19.1.0))(react@19.1.0):
    dependencies:
      '@expo/config-plugins': 55.0.8
      '@expo/config-types': 55.0.5
      '@expo/plist': 0.5.3
